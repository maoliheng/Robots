...
namespace Robots
{
	LegIII::LegIII(const char *Name, RobotTypeIII* pRobot)
		: LegBase(static_cast<RobotBase *>(pRobot), Name)
		, pRobot(pRobot)
	{
	}

	void LegIII::GetdJacOverPee(double *dJi_x, double *dJi_y, double *dJi_z, const char *relativeCoordinate) const
	{
	...
	void LegIII::GetFin(double *fIn) const
	{
		fIn[0] = f1_dyn + f1_frc;
		fIn[1] = f2_dyn + f2_frc;
		fIn[2] = f3_dyn + f3_frc;
	}
	void LegIII::GetFinDyn(double *fIn) const
	{
		fIn[0] = f1_dyn;
		fIn[1] = f2_dyn;
		fIn[2] = f3_dyn;
	}
	void LegIII::GetFinFrc(double *fIn) const
	{
		fIn[0] = f1_frc;
		fIn[1] = f2_frc;
		fIn[2] = f3_frc;
	...
	void LegIII::calculate_from_pEE()
	{
		_CalCdByPos();
		_CalVarByCd();
		_CalPartByVar();
	}
	void LegIII::calculate_from_pIn()
	{
		_CalCdByPlen();
		_CalVarByCd();
		_CalPartByVar();
	}
	void LegIII::calculate_from_vEE()
	{
		_CalVcdByVpos();
		_CalVvarByVcd();
		_CalVpartByVvar();
	}
	void LegIII::calculate_from_vIn()
	{
		_CalVcdByVplen();
		_CalVvarByVcd();
		_CalVpartByVvar();
	}
	void LegIII::calculate_from_aEE()
	{
		_CalAcdByApos();
		_CalAvarByAcd();
		_CalApartByAvar();
	}
	void LegIII::calculate_from_aIn()
	{
		_CalAcdByAplen();
		_CalAvarByAcd();
		_CalApartByAvar();
	}
	void LegIII::calculate_jac()
	{
	...
	void LegIII::calculate_diff_jac()
	{
	...
	RobotTypeIII::RobotTypeIII() : pLF{ &LF_Leg }, pLM{ &LM_Leg }, pLR{ &LR_Leg }, pRF{ &RF_Leg }, pRM{ &RM_Leg }, pRR{ &RR_Leg }
	{
		for (int i = 0; i < 6; ++i)
		{
	...
	void RobotTypeIII::GetFin(double *fIn) const
	{
		for (int i = 0; i < 6; ++i)
		{
	...
	void RobotTypeIII::GetFinDyn(double *fIn) const
	{
		for (int i = 0; i < 6; ++i)
		{
	...
	void RobotTypeIII::GetFinFrc(double *fIn) const
	{
		for (int i = 0; i < 6; ++i)
		{
	...
	void RobotTypeIII::SetFixFeet(const char* fixFeet)
	{
		for (int i = 0; i < 6; ++i)
		{
			if (fixFeet[i] == '0')
	...
	const char* RobotTypeIII::FixFeet() const
	{
	...
	void RobotTypeIII::SetActiveMotion(const char* activeMotion)
	{
		for (int i = 0; i < 6; ++i)
		{
			for (int j = 0; j < 3; ++j)
	...
	const char* RobotTypeIII::ActiveMotion() const
	{
	...
	{
		aris::dynamic::Model::loadXml(ele);
		...
		for (int j = 0; j < 6; j += 3)
		{
			pLegs[j]->base_mak_id_ = fbody().markerPool().find(pLegs[j]->name() + "_Base");

			pLegs[j]->u1i_ = fbody().markerPool().find(pLegs[j]->name() + "_U1i");
			pLegs[j]->u2i_ = fbody().markerPool().find(pLegs[j]->name() + "_U2i");
			pLegs[j]->u3i_ = fbody().markerPool().find(pLegs[j]->name() + "_U3i");
		}
		...
		for (int j = 2; j < 6; j += 3)
		{
			pLegs[j]->base_mak_id_ = rbody().markerPool().find(pLegs[j]->name() + "_Base");

			pLegs[j]->u1i_ = rbody().markerPool().find(pLegs[j]->name() + "_U1i");
			pLegs[j]->u2i_ = rbody().markerPool().find(pLegs[j]->name() + "_U2i");
			pLegs[j]->u3i_ = rbody().markerPool().find(pLegs[j]->name() + "_U3i");
		}
		...
		*const_cast<double *>(&theta) = std::atan(std::fabs((BM_R3y - BM_R1y) / (BM_R3z - BM_R1z)));

		for (int i = 0; i < 6; ++i)
		{
			double pm[4][4]{ 0 };
			aris::dynamic::s_inv_pm_dot_pm(*pLegs[i]->base().prtPm(), *pLegs[i]->u2i().prtPm(), *pm);
			*const_cast<double *>(&pLegs[i]->U2x) = pm[0][3];
			*const_cast<double *>(&pLegs[i]->U2y) = pm[1][3];
			*const_cast<double *>(&pLegs[i]->U2z) = pm[2][3];
			aris::dynamic::s_inv_pm_dot_pm(*pLegs[i]->base().prtPm(), *pLegs[i]->u3i().prtPm(), *pm);
	...
	auto RobotTypeIII::saveXml(aris::core::XmlElement &xml_ele)const->void
	{
	...
	void RobotTypeIII::SetWa(const double angle)
	{
		...
		double pe1[6]{ BM_R1x, BM_R1y, BM_R1z, 0, wa_, 0 };
		s_pe2pm(pe1, *pm);
		s_pm_dot_pm(*body().pm(), *pm, *pm_g);
		llink().setPm(*pm_g);

		double pe2[6]{ BM_R2x, BM_R2y, BM_R2z, 0, wa_, 0 };
		s_pe2pm(pe2, *pm);
		s_pm_dot_pm(*body().pm(), *pm, *pm_g);
		ulink().setPm(*pm_g);

		double pe3[6]{ 0, L_f*std::sin(wa_), -L_f*std::cos(wa_), 0, 0, 0 };
		s_pe2pm(pe3, *pm);
		s_pm_dot_pm(*body().pm(), *pm, *pm_g);
		fbody().setPm(*pm_g);

		double pe4[6]{ 0, -L_r*std::sin(wa_), L_r*std::cos(wa_), 0, 0, 0 };
		s_pe2pm(pe4, *pm);
		s_pm_dot_pm(*body().pm(), *pm, *pm_g);
		rbody().setPm(*pm_g);
